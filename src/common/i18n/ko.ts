...
export const ko: Strings = {
  ...
  itemDescriptions: {
    'rare-candy': '지금 포켓몬이 아직 완전히 진화하지 않았다면 즉시 진화시켜요.',
    'master-ball': '모든 세대 중에서 무작위 준전설, 전설 또는 환상의 포켓몬을 보여줘요.',
    'premier-ball': '모든 세대와 희귀도 중에서 무작위 포켓몬을 이로치로 보여줘요.',
  },
  ...
  hatchMessage: (name, cry, dropNote) => `${name}이(가) 포켓볼에서 부화했어요! ${cry}${dropNote}`,
  ...
  evolveMessageShiny: (name, cry) => `✨ 이로치 ${name}이(가) 진화했어요! ${cry}`,
  ...
  pokedexSnapshotCatchShiny: ' 그 계보를 계속 키우려면 이로치로 다시 잡아 주세요.',
  ...
  explorerModeChanged: 'Pokechi 모드가 탐색기로 바뀌었어요. 탐색기 보기에서 Pokechi를 찾아보세요!',
